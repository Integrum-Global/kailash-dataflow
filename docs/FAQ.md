...
**A:** DataFlow is a zero-config database framework built on Kailash Core SDK. It automatically generates 10 workflow nodes per model (CREATE, READ, UPDATE, DELETE, LIST, BULK_CREATE, BULK_UPDATE, BULK_DELETE, UPSERT, BULK_UPSERT). DataFlow is NOT an ORM - it's workflow-based with enterprise features like multi-tenancy and schema caching.
